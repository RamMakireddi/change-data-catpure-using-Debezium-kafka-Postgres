...
#Note: the -u denotes unbuffered (i.e., output string to stdout without buffering data and then writing to stdout)
...
def parse_payload(input_raw_json):
    ...
    if op_type == 'c':
        return parse_create(input_json.get('payload', {}).get('after', {}), op_type)
    elif op_type == 'd':
        return parse_delete( input_json.get('payload', {}).get('before', {}), input_json.get('payload', {}).get('ts_ms', None), op_type)
    elif op_type == 'u':
        return parse_update( input_json.get('payload', {}), op_type)

    #no need to write log events
    return []

for line in sys.stdin:
    #1. reads line from unix pipe, assume only valid json comes through
    #2. parse the payload into a format we can use
    #3. prints out the formatted data as a string to stdout
    #4. the string format is holding_id, user_id, change_field, old_value, new_value, datetime_created, datetime_updated, datetime_deleted, datatime_inserted
